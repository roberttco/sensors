...
int ConnectToWiFi(int retries, IPAddress ip, IPAddress dns, IPAddress gw, IPAddress subnet)
{
  ...
  int connectDelay = 3500;  // observerd typical connect time is about 13/.25 seconds.
  ...
  for (int wap = 0; wap < NUMWAPS; wap++)
  {
    ...
    while ((status != WL_CONNECTED) && ( my_retries > 0))
    {
      ...
      delay(connectDelay);
      connectDelay = 250; // after initial delay retry every .25 seconds.
      my_retries--;
    }
...
void setup()
{
  ...
  if (whichap == -1)
  {
    DEBUG_PRINTLN("Connect retries exhausted.  Restarting.");
    ESP.restart();
  }
  ...
  float vdd = (ESP.getVcc() / 1000.0) + VCC_CORRECTION;
  ...
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    // for deep sleep mode we dont care what the return code is
  ...
  if (dontSleep == false)
  {
    DEBUG_PRINTLN ("Deep-sleeping for 60 seconds...");
